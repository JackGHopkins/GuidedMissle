...
const float UMyProjectileMovementComponent::MIN_TICK_TIME = 1e-6f;

UMyProjectileMovementComponent::UMyProjectileMovementComponent(const FObjectInitializer& ObjectInitializer)
...
void UMyProjectileMovementComponent::PostLoad()
{
	Super::PostLoad();
...
void UMyProjectileMovementComponent::InitializeComponent()
{
...
void UMyProjectileMovementComponent::UpdateTickRegistration()
{
...
void UMyProjectileMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool UMyProjectileMovementComponent::HandleDeflection(FHitResult& Hit, const FVector& OldVelocity, const uint32 NumBounces, float& SubTickTimeRemaining)
{
...
bool UMyProjectileMovementComponent::HandleSliding(FHitResult& Hit, float& SubTickTimeRemaining)
{
...
void UMyProjectileMovementComponent::SetVelocityInLocalSpace(FVector NewVelocity)
{
	if (UpdatedComponent)
	{
...
FVector UMyProjectileMovementComponent::ComputeVelocity(FVector InitialVelocity, float DeltaTime) const
{
	// v = v0 + a*t
...
FVector UMyProjectileMovementComponent::LimitVelocity(FVector NewVelocity) const
{
...
FVector UMyProjectileMovementComponent::ComputeMoveDelta(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UMyProjectileMovementComponent::ComputeAcceleration(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UMyProjectileMovementComponent::ComputeHomingAcceleration(const FVector& InVelocity, float DeltaTime) const
{
...
void UMyProjectileMovementComponent::AddForce(FVector Force)
{
	PendingForce += Force;
}

void UMyProjectileMovementComponent::ClearPendingForce(bool bClearImmediateForce)
{
...
float UMyProjectileMovementComponent::GetGravityZ() const
{
...
UMyProjectileMovementComponent::EHandleBlockingHitResult UMyProjectileMovementComponent::HandleBlockingHit(const FHitResult& Hit, float TimeTick, const FVector& MoveDelta, float& SubTickTimeRemaining)
{
...
FVector UMyProjectileMovementComponent::ComputeBounceResult(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
...
void UMyProjectileMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
...
bool UMyProjectileMovementComponent::CheckStillInWorld()
{
	if (!UpdatedComponent)
	{
		return false;
...
bool UMyProjectileMovementComponent::ShouldUseSubStepping() const
{
	return bForceSubStepping || (GetGravityZ() != 0.f) || (bIsHomingProjectile && HomingTargetComponent.IsValid());
}


float UMyProjectileMovementComponent::GetSimulationTimeStep(float RemainingTime, int32 Iterations) const
{
...
void UMyProjectileMovementComponent::SetInterpolatedComponent(USceneComponent* Component)
{
...
USceneComponent* UMyProjectileMovementComponent::GetInterpolatedComponent() const
{
	return InterpolatedComponentPtr.Get();
}

void UMyProjectileMovementComponent::MoveInterpolationTarget(const FVector& NewLocation, const FRotator& NewRotation)
{
	if (!UpdatedComponent)
	{
		return;
...
void UMyProjectileMovementComponent::ResetInterpolation()
{
	if (USceneComponent* InterpComponent = GetInterpolatedComponent())
	{
...
void UMyProjectileMovementComponent::TickInterpolation(float DeltaTime)
{
	if (!bInterpolationComplete)
	{
		if (USceneComponent* InterpComponent = GetInterpolatedComponent())
